  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h2 class="mt-1 text-3xl font-bold tracking-tight text-slate-900">Edit Job Category</h2>
        <p class="mt-2 text-sm text-slate-600">Update the category name.</p>
      </div>
    ...
    <div v-else class="mx-auto max-w-2xl rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-lg shadow-slate-950/5 backdrop-blur sm:p-8">
...
const fetchCategory = async () => {
  ...
  try {
    const response = await api.get(`/job-categories/${route.params.id}`)
    const category = response.data.data
    form.value.name = category?.name || ''
  } catch (err: any) {
    formError.value = err.response?.data?.message || 'Failed to load category'
  } finally {
    loading.value = false
  }
}
...
const updateCategory = async () => {
  ...
  try {
    await api.put(`/job-categories/${route.params.id}`, {
      name: form.value.name
    })

    router.push('/admin/job-categories')
  } catch (err: any) {
    formError.value = err.response?.data?.message || 'Failed to update category'
