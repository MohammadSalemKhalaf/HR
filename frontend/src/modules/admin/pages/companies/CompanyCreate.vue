  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h2 class="mt-1 text-3xl font-bold tracking-tight text-slate-900">Create Company</h2>
        <p class="mt-2 text-sm text-slate-600">Add a new company profile to the system.</p>
      </div>
    ...
    <div class="mx-auto max-w-3xl rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-lg shadow-slate-950/5 backdrop-blur sm:p-8">
      <form @submit.prevent="createCompany" class="space-y-6">
        ...
        <div class="grid gap-6 md:grid-cols-2">
          <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Owner Email (used for login)</label>
            <input
              v-model="form.email"
              type="email"
              class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100"
          ...
          <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Owner Password (optional)</label>
            <input
...
// owner email/password for company user
form.value.email = ''
form.value.password = ''
...
const createCompany = async () => {
  ...
  try {
    ...
    formError.value = err.response?.data?.message || 'Failed to create company'
